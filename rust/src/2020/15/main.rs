...
fn next_number(map: &mut HashMap<usize, (usize, usize)>, (turn, last): (usize, usize)) -> usize {
...
fn main() {
    ...
    while turn < 2020 {
        let new_number = next_number(&mut map, (turn, last));
        turn += 1;
        last = new_number;
    }
    dbg!(last);
    while turn < 30000000 {
        let new_last = next_number(&mut map, (turn, last));
        turn += 1;
        last = new_last;
    }
    dbg!(last);
